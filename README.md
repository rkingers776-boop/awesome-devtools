...
- [CoreTechX](https://www.jslet.com/) — 51 client-side engineering calculators for DevOps, SRE, and infrastructure. GPU inference latency & training cost, DNS propagation (8 resolver populations), K8s pod density & container resource limits, database connection pool sizing, Docker image pull time, CI pipeline queue time, CDN cache hit ratio, VPN throughput, latency budget decomposition, SLA economics, password entropy, and cloud egress cost. All single HTML files, 100% browser-side — no npm, no build step, no signup.
